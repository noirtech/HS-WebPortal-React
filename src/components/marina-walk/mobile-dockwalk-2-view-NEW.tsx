...
import { Ship, MapPin, Search, CheckCircle, AlertTriangle, Clock, RefreshCw, X, Anchor, Eye, BarChart3, Wrench, FileText, Download, Share, Bell, Settings, User, Shield, Globe, Database, HelpCircle, Zap, Brain, Box, TrendingUp, Users, Mic, Camera, Upload, Copy, Trash2, Droplets, ArrowLeft, ChevronDown } from 'lucide-react'
import { useLocaleFormatting } from '@/lib/locale-context'
import { useTooltip } from '@/hooks/useTooltip'
import { logger } from '@/lib/logger'

// Configuration constants
const SHOW_MOUSEOVER_LABELS = true
...
export default function MobileDockwalk2View({ berths, boats }: MobileDockwalk2ViewProps) {
  ...
  // Force hide tooltip when showLabels changes to false
  useEffect(() => {
    logger.debug('showLabels effect triggered', { showLabels, tooltipVisible: tooltipState.isVisible })
    if (!showLabels) {
      logger.debug('Calling hideTooltip due to showLabels = false')
      hideTooltip()
    }
  }, [showLabels, hideTooltip, tooltipState.isVisible])
          ...
          {enrichedBerths.map((berth) => {
            ...
            return (
                 ...
                 <div className="flex items-center justify-between">
                   ...
                   {berth.boatName ? (
                     <div className="p-1.5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded border border-blue-200 w-fit" data-element-name={`boat-info-${berth.id}`}>
                     ...
                     <div className="p-1.5 bg-gray-50 rounded border border-gray-200 w-fit" data-element-name={`no-boat-info-${berth.id}`}>
                       <div className="flex items-center space-x-1.5">
                         ...
                         <span className="text-xs text-gray-600">No boat assigned</span>
                                       ...
                                       {/* Action Buttons - Dropdown for A1, regular buttons for others */}
                    {berth.id === 'A1' ? (
                      <div className="relative berth-dropdown ml-auto" data-element-name={`action-buttons-${berth.id}`}>
                        <Button
                          onClick={() => setOpenDropdown(openDropdown === berth.id ? null : berth.id)}
                          className={`h-6 px-2 text-xs ${getBerthButtonStyle(berth.id)} text-white transition-all duration-200 flex items-center gap-1`}
                          data-element-name={`verify-boat-button-${berth.id}`}
                          onMouseEnter={(e) => {
                            logger.debug('Verify button mouseEnter', { showLabels })
                            showLabels && showTooltip('Click to verify boat status', e.clientX, e.clientY)
                          }}
                          onMouseLeave={hideTooltip}
                        >
                          {React.createElement(getBerthButtonIcon(berth.id), { className: 'h-3 w-3' })}
                          {getBerthButtonText(berth.id)}
                          <ChevronDown className="h-3 w-3 ml-1" />
                        </Button>

                        {/* Dropdown Menu */}
                        {openDropdown === berth.id && (
                          <div className="absolute top-full right-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                            <div className="py-1">
                              ...
                              >
                                ...
                                Select a different boat for this berth
                              </button>
                              <button
                      ...
                      <div className="flex gap-1" data-element-name={`action-buttons-${berth.id}`}>
                        <Button
                          ...
                          onMouseEnter={(e) => {
                           logger.debug('Confirm button mouseEnter', { showLabels })
                           showLabels && showTooltip('Confirm this boat is correct', e.clientX, e.clientY)
                          }}
                          onMouseLeave={hideTooltip}
                        >
                        ...
                        <Button
                          ...
                          onMouseEnter={(e) => showLabels && showTooltip('Select a different boat for this berth', e.clientX, e.clientY)}
                          onMouseLeave={hideTooltip}
                        >
                        ...
                        <Button
                          ...
                          onMouseEnter={(e) => showLabels && showTooltip('Mark this berth as empty', e.clientX, e.clientY)}
                          onMouseLeave={hideTooltip}
                        >
                   ...
                   {berthState && (
                     <div className="mt-2 pt-2 border-t border-gray-200">
                          ...
                          onMouseEnter={(e) => showLabels && showTooltip('Undo the previous action', e.clientX, e.clientY)}
                          onMouseLeave={hideTooltip}
                        >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
          ...
          <div className="flex items-center justify-between py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button
                ...
                } text-white shadow-md text-xs sm:text-sm relative`}
                        onClick={() => {
          logger.debug('Hide/Show Labels button clicked', { currentShowLabels: showLabels, willSetTo: !showLabels })
          setShowLabels(!showLabels)
        }}
                data-element-name="show-labels-button"
              >
      ...
      {showBoatSelectionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div
            ...
            className="bg-white rounded-xl shadow-2xl p-6 w-96 max-h-96 overflow-hidden cursor-grab active:cursor-grabbing border-0"
      ...
      {/* Tooltip */}
      {(() => {
        const shouldShow = showLabels && tooltipState.isVisible
        logger.debug('Tooltip render check', { showLabels, tooltipVisible: tooltipState.isVisible, shouldShow, content: tooltipState.content })
        return shouldShow
      })() && (
        <div
          className="fixed z-50 px-3 py-2 text-sm text-white bg-gray-900 rounded-lg shadow-lg pointer-events-none transition-all duration-200"
          style={{
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-96 max-h-96 overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-gray-900">Show Element Labels</span>
                <Button
                  variant={showLabels ? "default" : "outline"}
                  size="sm"
                  onClick={() => setShowLabels(!showLabels)}
                  className={showLabels ? "bg-blue-600 hover:bg-blue-700" : ""}
                >
                  {showLabels ? "On" : "Off"}
                </Button>
              </div>
